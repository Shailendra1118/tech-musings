...
@Slf4j
public class RegularCustomerMinDiscountCalculator implements DiscountCalculator {

    private DiscountCalculator nextDiscountCalculator;
    private static final AmountSlab slab = AmountSlab.builder().startAmt(0).endAmt(5000)
            .discount(Discount.ZERO_PERCENT).build();
    ...
    public double calculate(PurchaseDetails purchaseDetails, double discountAmtTillNow) {
        ...
        if(purchaseDetails.getAmount() >= slab.getEndAmt()){
            ...
            log.info("Discount with first Slab={}", discountHere);
